...
class ViewAccountsPage(tk.Frame):
    COLS = ["ID", "Account No.", "Customer", "Phone", "Balance (PKR)", "Opened On"]
    ...
    def _build_toolbar(self):
        ...
        e = ctk.CTkEntry(bar, placeholder_text="🔍  Search account number or customer…",
                         textvariable=self._sv, fg_color=WHITE,
                         border_color=GOLD, border_width=1,
                         text_color=T1, placeholder_text_color=T2, height=36, corner_radius=8)
    ...
    def refresh(self):
        all_accs = self.acc_m.get_all_accounts() or []
        self._all_rows = []
        for a in all_accs:
            b = a.get("BRANCH_CODE") or a.get("REF_BRANCH_CODE")
            if b == self.branch_code or b is None:
                self._all_rows.append(a)
        self._sv.set("")
        self._render(self._all_rows)
        total = sum(float(r.get("CURRENT_BALANCE", 0) or 0) for r in self._all_rows)
